...
% add a custom float environment for listing in figure format
...
simple and reliable method for future weather data prediction and is widely
...
Therefore, it is necessary to use further weather data considering climate
...
the morphing method, a relatively simple and reliable statistical downscaling
...
while preserving historical weather sequences. Multiple future weather
...
based on CMIP6 ScenarioMIP climate scenarios, and develops a a free and
...
\begin{figure}
\includegraphics[width=\columnwidth]{figures/ssp} \caption{Scenario design matrix for future climate prediction based on SSP  and RCP in ScenarioMIP\citep{oneill2016scenario}}\label{fig:ssp}
\end{figure}
...
\begin{listing}
\caption{Install epwshiftr}\label{lst:install}
...
\begin{figure}
\includegraphics[width=\columnwidth]{figures/workflow} \caption{The primary process of using epwshiftr to generate EPW files for future weather}\label{fig:workflow}
\end{figure}
...
ESGF and thus is capable of querying all CMIP6 GCM outputs based on various
...
output. Also, an output index is generated based on the query and is used to
check the data integrity before extracting climate change data.

Listing 2 demonstrates how to use the \texttt{index\_cmip6\_index} interface from the
\emph{Query Module}.
...
ScenarioMIP activity for year 2050 and 2080. Also an output index file named
``cmip6\_index.csv'' will be saved.

\begin{listing}
\caption{Query ScenarioMIP outputs using the Query Module}\label{lst:index}
\includegraphics[width=\columnwidth]{figures/code-index.png}
\end{listing}

\hypertarget{gcm-raw-output-data-management}{%
...
change. Since the amount of data to extract from the global attributes in NetCDF
is negligible compared to the total data volume of the time-series matrix, the
computational load is small. It should be pointed out that CMIP6 does not
...
ESGF and NetCDF files to ensure they are all transformed to the standard
calendar.
...
the loaded files against index so that epwshiftr knows which case is complete
and can be used for the next step.

\begin{listing}
\caption{Summarize the GCM NetCDF files based on the output index}\label{lst:db}
\includegraphics[width=\columnwidth]{figures/code-db.png}
\end{listing}

\hypertarget{extraction-of-raw-climate-change-data-based-on-geographic-location}{%
...
global grid. The common methods are: (a) Euler distance method, (b) Tunnel
Distance method and (c) K-D tree algorithm. The Data Extraction Module uses the
...
\begin{listing}
\caption{Match the global grid points directly using an EPW file and extract raw GCM data}\label{lst:data}
...
the matched grid points and selected location should be less than 0.5 degree,
and only return the closest point. It also showcases the ability of directly
using an EPW file as input for coordinate matching, as epwshiftr can parse the
...
is often close to, if not greater than, a normal PC's memory. For this reason,
epwshiftr supports storing the original data in the fst format \citep{mark2022fst}
with a super large compression ratio, which greatly reduces the content burden
...
interval. All EPW files will be saved into a seprate folder.

\begin{listing}
\caption{Perform morphing and generate future EPWs}\label{lst:epw}
...
worldwide. This tool will significantly save the time and cost of obtaining
future weather data and thus can support the analysis of future building heating
and air-conditioning demand, energy consumption, and carbon emissions.
