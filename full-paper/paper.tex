...
\captionof{code}{Install epwshiftr}\label{lst:install}
...
\captionof{code}{Query ScenarioMIP outputs using the Query Module}\label{lst:index}
...
\captionof{code}{Summarize the GCM NetCDF files based on the output index}\label{lst:db}
...
\captionof{code}{Match the global grid points directly using an EPW file and  extract raw GCM data}\label{lst:data}
...
\captionof{code}{Perform morphing and generate future EPWs}\label{lst:epw}
